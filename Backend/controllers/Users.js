...
export const Register = async (req, res) => {
  ...
  if(!username || !email || !password || !confPassword) return res.status(400).json({ message: "Semua field harus diisi" });
  ...
  try {
    ...
    if (error instanceof Sequelize.UniqueConstraintError) {
      return res.status(400).json({ message: "Email sudah dipakai (constraint)" });
    }
...
export const Login = async(req, res) => {
    ...
    try {
        ...
        const refreshToken = jwt.sign({userId, username, email}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: '5d'});
        ...
        return res.status(200).json({accessToken, user: {userId, username, email}});
    } catch (error) {
...
export const Logout = async(req, res) => {
    ...
    return res.sendStatus(200).json({message: "Logout berhasil"});
}